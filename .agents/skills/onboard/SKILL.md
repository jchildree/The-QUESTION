...
description: >
  Interactive first-run experience for new Codex-ITect installs. Asks 3 short
...
1. Check if `.Codex/.onboard-seen` exists in the project root.
...
**Question 3 of 3 -- How should Codex talk to you?**
...
1. Write `.Codex/.onboard-seen` in the project root containing an ISO timestamp and the 5 recommended skill names, one per line:
   ```
...
6. Execute the `/setup-scythe-skills` skill automatically if it was not setup for this project.

## Boundaries

- Never re-run the full questionnaire without `--reset` flag.
- Never suggest more than 5 skills.
- If a skill name from the routing table does not exist in `.Codex/skills/`, substitute `audit` as fallback.
