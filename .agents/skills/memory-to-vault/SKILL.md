...
dependencies: []
description: >
  Convert Codex's persisted memory into a structured high-level outline and
...
Reads Codex's persisted memory for the current project and generates a categorized high-level outline, saved as a vault note.
...
```powershell
# Windows
& ".Codex/skills/memory-to-vault/scripts/memory-to-outline.ps1"
```

```bash
# Unix -- use equivalent shell logic
hash=$(pwd | sed 's|/|-|g; s|^-||')
memdir="$HOME/.Codex/projects/$hash/memory"
```
...
[[Codex-ITect-Skill Update Index]]
```
