...
"The Question" is a Codex plugin (not an application). It has no build, no test
...
- `the-question/the-question/` is the live plugin root (`${CLAUDE_PLUGIN_ROOT}`).
  Contains `.Codex-plugin/plugin.json`, the shared docs (`QUESTION.md`, `BOARD.md`), and
