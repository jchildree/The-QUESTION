...
process.stdin.on("end", () => {
  try {
    ...
    if (
      /\b(activate|enable|turn on|start|talk like)\b.*\bcaveman\b/i.test(
        prompt
      ) ||
    ...
    const statsMatch = /^\/caveman(?::caveman)?-stats(?:\s+(.*))?$/.exec(
      prompt
    );
    if (statsMatch) {
      const tailArgs = (statsMatch[1] || "")
        .trim()
        .split(/\s+/)
        .filter(Boolean);
      try {
        ...
        if (data.transcript_path)
          argv.push("--session-file", data.transcript_path);
        ...
        process.stdout.write(
          JSON.stringify({ decision: "block", reason: out.trim() })
        );
      } catch (e) {
        process.stdout.write(
          JSON.stringify({
