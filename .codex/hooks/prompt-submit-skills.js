...
const PARALLEL_PATTERN =
  /\b(parallel|concurrent|subagents?|dispatch(ing)?|independent tasks?|in parallel|multiple agents?|run agents?|spawn agents?|agent pool|fan.?out)\b/i;
...
process.stdin.on("end", () => {
  ...
  if (PARALLEL_PATTERN.test(prompt)) {
    ...
    if (sdd)
      sections.push("=== SKILL: subagent-driven-development ===\n" + sdd);
  }
