...
**Emit (per `${CLAUDE_PLUGIN_ROOT}/BOARD.md` provenance):** pin each Suspect
as a `hypothesis` node with `verify:` set to the experiment that would
confirm it and `confidence: unverified`. A Suspect surfaced from a prior
Case via cross-case recall keeps its original `id:`, adds `#federated`, and
inherits its confidence -- never re-derived.
...
**Emit:** when two or more independent probes or sources agree, update the
node to `confidence: corroborated` (amber). Stays provisional until `verify`
passes.
...
**Emit:** the moment a Suspect's `verify` passes (reproduced / `Confirmed`),
promote it to `confidence: verified` (green). The fix-gate's precondition is
a green root-cause finding; user consent is its trigger. No green, no fix.
